...
public class MyFitnessFunction  {
    ...
    //pipeline pair处理
    public void pipelinePairSameVm(int assign1[]) {
    	List<ArrayList<Integer>> pipelineTaskList =  Tool.pipelineTaskList;
    	...
    	//从前 或者从后 设置vm  近似解决的是一条链下面全是pipepair  即 要么child跟着parent  要么parent跟着child  随机的滚动
    	if (random.nextDouble() > 0.5 ) {
	    	for(int i=0;i<pipelineTaskList.size();i++) {
	    		pipeList = pipelineTaskList.get(i);
	    		taskIndex1 = pipeList.get(0).intValue() - 1;
	    		taskIndex2 = pipeList.get(1).intValue() - 1;
	    		vmIndex1 = assign1[taskIndex1];
	    		vmIndex2 = assign1[taskIndex2];
	    		
	    		//处理
	    		assign1[taskIndex2] = vmIndex1;
	    		
	    	
	    	}
    	}else {//从下往上
	    		for(int i=pipelineTaskList.size()-1;i>=0;i--) {
		    		pipeList = pipelineTaskList.get(i);
		    		taskIndex1 = pipeList.get(0).intValue() - 1;
		    		taskIndex2 = pipeList.get(1).intValue() - 1;
		    		vmIndex1 = assign1[taskIndex1];
               ...
               // max(availTime,max(FTti))
                ...
                for (Task parrentTask : task.getParentList()) {
                    ...
                    if (vmId != parentVmId) {
                        ...
                        //计算传输量 
                        vmParTimeTran.transmission = vmParTimeTran.transmission + getOutputSize(parrentTask);
                        vmTimeTranList.set(parentVmId, vmParTimeTran);//保存数据
                        vmTimeTran.transmission = vmTimeTran.transmission + getOutputSize(parrentTask);
                    }
...
//    }
    ...
    public double makespanDivideDeadline() {
    	double makespan = calMakespan();
    	double deadline1 = deadline;
    	return makespan/deadline1;
    }
    
    //计算cost/budget
    public double costDivideBudget() {
    	double cost = calCost();
    	double budget1 = budget;
    	return cost/budget1;
    }
    
    //计算energy/rand
    public double energyDivideRandEnergy() {
    	double energy1 = calEnergyConsumption();
    	double randEnergy = energy;
    	return energy1/randEnergy;
    }
}
